 ...
 *   @desc:     { 更新url阅读数 } 
 */
module.exports.addCommonVisitCount = async url => await axios({
 ...
 *   @desc:     { 查询县级单位邮政编码 } 
 */
module.exports.getVoteCount = async county_id => await axios({
...
*/
module.exports.addCommonErrorLog = async params => await axios({
    url: '/22/6768cba468.json',
    params,
}).then(res => res);

// 20W奖品/2000W人，中奖率1%;
module.exports.doLottery = (rate = 5) => 100 * Math.random() <= rate ? 1 : 0;
...
*/
// module.exports.addVoteMain = async params => await axios({
...
*   @desc:     { 提交反假答题数据 } 
    const { sid, nickname, openid, sex, headimgurl, county_id,  vote_detail, score, prize_id, time_used, rec_date, dom_content_loaded, loaded, net_type, os_version, wechat_version } = params;
*/
module.exports.submitPaper = async params => await axios({
        url: "/addpaper",
        params,
    }).then(res => res)
    .catch(e => {
 ...
 *   @desc:     { 抽奖逻辑 } 
 */
module.exports.getBasePrize = async() => await axios({
...
*/
module.exports.addVoteLuckyUser = async params => await axios({
...
*/
module.exports.setVoteLuckyUser = async params => await axios({
